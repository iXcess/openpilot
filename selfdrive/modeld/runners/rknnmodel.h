...
struct RKNNModelInput : public ModelInput {
  ...
  RKNNModelInput(const std::string _name, float *_buffer, int _size, float *_rknn_buffer)
    : ModelInput(_name, _buffer, _size), rknn_buffer(std::move(_rknn_buffer)) {}
...
public:
  ...
  ~RKNNModel();  // Destructor declaration
...
private:
  ...
  rknn_input rknn_inputs[9];
  rknn_output rknn_outputs[1];
  rknn_tensor_mem* model_mem;
};
